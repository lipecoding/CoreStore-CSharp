...
namespace CoreStore_CSharp.Dao
{
    class UserDao
    {
        ...
        public void create(string user, string pass, string name, string lname, string adress, string zip, string cpf, string birth, int adm) {
            con = new ConSql();
            cmd = new MySqlCommand();

            cmd.CommandText = "insert into users (username,pass,name,lastname,adress,zipcode,cpf,birthday,admin) values (@u,@p,@n,@l,@a,@z,@c,@b,@admin)";
            cmd.Parameters.AddWithValue("@u", user);
            cmd.Parameters.AddWithValue("@p", pass);
            cmd.Parameters.AddWithValue("@n", name);
            cmd.Parameters.AddWithValue("@l", lname);
